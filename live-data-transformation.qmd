...
sanitation \|\> select(name, starts_with("p"))sanitation \|\> select(name, starts_with("p"))Helpers select variables by matching patterns in their names:
...
sanitation |> 
  relocate(income_id, .before = residence)
```
