...
```{r}
global_waste_data <- read_csv("data/country_level_data_0.csv")
...
```{r}
subset_na_food_organic_waste <- composition_tidy |> 
...
subset_not_LIC <- composition_tidy |> 
...
subset_HIC_foodandwastegreaterthan_50percent <- composition_tidy |> 
...
```{r}
subset_HICandLIC_othergreaterthan_50percent <- composition_tidy |> 
  filter(income_id == "LIC" | income_id == "HIC",
         other > 40 )
```
...
```{r}
subset_part6 <- composition_tidy |> 
  filter(country_name %in%  c("Switzerland", "Ghana", "Kenya", "Turkey", "Nepal", "United States"))
```
...
The mutate() function creates a new column or variable, while case_when() sets specific paramters inside the mutate function. It says that within the income_id column, when a value is set to LIC or HIC or whatever it is, make the corresponding value in income_cat column be the ful spelling of the income Ids. It is setting up parameters for creating the new column.

3.  How is the object `levels_income_cat` used and what does it achieve?

levels_income_cat is a vector containing the new income descriptions. It is defining specific orders to the levels, like ranking them from highest to lowest in the "levels = levels_income_cat" line.
...
The resulting table shows me how many of each country, the count, are in each income group.
...
The difference between this table and the last is that the descriptions hae changed from the abbreviated versions ("LIC" "HIC" and so on) to their written forms from the income_cat object. The new table also goes in order form highest to lowest income, while the previous table had no clear ranking.
...
It removed the na from the high income category, so now there are 79 count instead of 81.
...
table_msw_generation <- composition_tidy_capita_income_cat |> 
  ...
  group_by(income_cat) |> 
  summarize(count = n(),
            mean(msw_capita_kg_year),
            median(msw_capita_kg_year),
            sd(msw_capita_kg_year))
```
...
#table_composition <- 
composition_tidy_capita_income_cat |> 
  tidyr::drop_na(c(msw_capita_kg_year,
                   food_organic_waste,
                   plastic,
                   paper_cardboard)) |> 
  group_by(income_cat) |> 
...
-   `gt()`: Makes the table asthetically pleasing and easier to export
-   `tab_header()`: allows to create your table header and subheader
-   `fmt_number()`: selecting which columns to use and sets a decimal limit of 0.1
-   `starts_with()`: gets every column that starts with (mean)
-   `cols_labels()`: puts column titles associated with each variable grabbed
...
-   `rename()`: renames the columns to a more visually appealing output with spaces
-   `knitr::kable()`: this automatically converts your data and descriptions into an attractive output
